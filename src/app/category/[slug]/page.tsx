  ...
  return (
    <div className="container-x py-8">
      <nav className="text-sm text-brand-700 mb-2">
        <a href="/">Home</a> / <span>{cat.name}</span>
      </nav>
      <h1 className="font-display text-3xl text-brand-900 mb-6">{cat.name}</h1>
      {cat.products.length === 0 ? (
        <p className="text-brand-700">No products in this category yet.</p>
      ) : (
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
          {cat.products.map((p) => <ProductCard key={p.id} p={p} />)}
        </div>
      )}
    </div>
  );
