  ...
  return (
    <div className="container-x py-8 grid lg:grid-cols-3 gap-6">
      <aside className="card p-5 h-fit">
        ...
        {effective > 0 && (
          <div className="mt-3 p-2 bg-brand-50 rounded border border-brand-200 text-sm">
            🎉 {t("member_discount_label")}: <strong>{effective}%</strong>
          </div>
        )}
      ...
      <section className="lg:col-span-2 space-y-3">
        <h1 className="font-display text-2xl text-brand-900">{t("my_orders")}</h1>
        {orders.length === 0 ? (
          <div className="card p-6 text-center">
            <p className="text-brand-700 mb-3">{t("no_orders")}</p>
            <Link href="/" className="btn-primary">{t("start_shopping")}</Link>
          </div>
        ) : (
          <div className="card overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="bg-brand-50"><tr className="text-left">
                <th className="p-2">{t("order_number")}</th><th className="p-2">{t("date")}</th><th className="p-2">{t("total")}</th><th className="p-2">{t("status")}</th>
              </tr></thead>
              <tbody>
                {orders.map((o) => (
                  <tr key={o.id} className="border-t border-brand-100">
                    ...
                    <td className="p-2"><span className="px-2 py-0.5 rounded bg-brand-100 text-xs">{t(o.status as any)}</span></td>
                  </tr>
                ))}
              </tbody>
            </table>
