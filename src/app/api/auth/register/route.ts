...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!normPhone) return NextResponse.json({ error: "Invalid Sri Lankan phone number" }, { status: 400 });
    ...
    const user = await prisma.user.create({
      data: { phone: normPhone, fullName: String(fullName), email: email ? String(email) : null, passwordHash }
    });
